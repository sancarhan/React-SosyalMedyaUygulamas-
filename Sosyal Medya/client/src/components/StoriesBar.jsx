      ...
      <div className='flex gap-4 pb-5'>
       ...
       border-indigo-300 bg-gradient-to-b from-indigo-50 to-white'>
        ...
        <div className='h-full flex flex-col items-center justify-center p-4'>
         ...
         <p className='text-sm font-medium text-slate-700 text-center'>Hikaye Paylaş</p>
        </div>
       </div>
        ...
        stories.map((story, index)=> (
         ...
         >
          ...
          <p className='absolute top-18 left-3 text-white/60 text-sm
           ...
           text-xs'>{moment(story.createdAt).locale("tr").fromNow()}</p>
           {
            story.media_type != 'text' &&(
              <div className='absolute inset-0 z-1 rounded-lg bg-black overflow-hidden'>
                {
            story.media_type === "image" ?
            <img src={story.media_url} alt="" className='h-full w-full object-cover hover:scale-110 transition duration-500 opacity-70
            hover:opacity-80'/>
            :
